...
# %%
#begin creating the dashboard frame as db_new_v1

#count number of unique patients per device
db_new_v1 = joined_updated[['device','patient_id', 'assigned_sex']].groupby(by=['device','patient_id']).count().reset_index().groupby(by='device').count().reset_index()
db_new_v1 = db_new_v1.rename(columns={'patient_id':'Unique Subjects'})
...
for i in ['Female','Male']:
    ...
    db_new_v1 = db_new_v1.merge(tdf, left_on='device', right_on='device', how='outer')
    db_new_v1.rename(columns={'patient_id':i}, inplace=True)
...
for i in [mstlight, mstmedium, mstdark]:
    ...
    db_new_v1 = db_new_v1.merge(tdf, left_on='device', right_on='device', how='outer')
    db_new_v1.rename(columns={'patient_id':'monk_forehead_'+i[0]}, inplace=True)
...
# db_new_v1 = db_new_v1.merge(stats('age_at_session',joined_updated), left_on='device', right_index=True, how='outer')
# db_new_v1 = db_new_v1.merge(stats('bmi',joined_updated), left_on='device', right_index=True, how='outer')

########## add device names and priority
# merge with devices, keeping all devices
db_new_v1 = devices[['manufacturer','model','priority']].merge(db_new_v1, left_index=True, right_on='device', how='outer').reset_index().drop(columns=['index'])
db_new_v1 = db_new_v1.rename(columns={'manufacturer':'Manufacturer', 'model':'Model'})
...
# merge db_new_v1 with tdf, if there is no session with >=25% of so2 in the 3 decades, fill the value with 0
db_new_v1 = db_new_v1.merge(tdf, left_on='device', right_on='device', how='outer')

# %%
# fill zeroes
db_new_v1.fillna(0, inplace=True)
# drop the assigned_sex column in db_new_v1
db_new_v1 = db_new_v1.drop(columns=['assigned_sex'])
