...
# %%
#begin creating the dashboard frame as db_old

#count number of unique sessions per device
db_old = joined_updated[['device','patient_id', 'assigned_sex', 'fitzpatrick_x','session']].groupby(by=['device','session']).count().reset_index().groupby(by='device').count().reset_index()
db_old = db_old.rename(columns={'session':'Unique Sessions'})
...
for i in ['Female','Male']:
    ...
    db_old = db_old.merge(tdf, left_on='device', right_on='device', how='outer')
    db_old.rename(columns={'patient_id':i}, inplace=True)
...
db_old = db_old.merge(tdf, left_on='device', right_on='device', how='outer')
db_old.rename(columns={'patient_id':'Fitzpatrick_5_6'}, inplace=True)
...
for i in [mstlight, mstmedium, mstdark]:
    ...
    db_old = db_old.merge(tdf, left_on='device', right_on='device', how='outer')
    db_old.rename(columns={'patient_id':'monk_forehead_'+i[0]}, inplace=True)
...
for i in [mst_1_2, mst_3_4, mst_5_6, mst_7_8, mst_9_10]:
    tdf = joined_updated[joined_updated['monk_forehead'].isin(i)].groupby(by=(['device','patient_id'])).count().reset_index().groupby('device').count()['patient_id']
    db_old = db_old.merge(tdf, left_on='device', right_on='device', how='outer')
    db_old.rename(columns={'patient_id':'monk_forehead_'+i[0]+i[1]}, inplace=True)
...
# db_old = db_old.merge(stats('age_at_session',joined_updated), left_on='device', right_index=True, how='outer')
# db_old = db_old.merge(stats('bmi',joined_updated), left_on='device', right_index=True, how='outer')

########## add device names and priority
# merge with devices, keeping all devices
db_old = devices[['manufacturer','model','priority']].merge(db_old, left_index=True, right_on='device', how='outer').reset_index().drop(columns=['index'])
db_old = db_old.rename(columns={'manufacturer':'Manufacturer', 'model':'Model'})
...
joined_updated['sample_range'] = joined_updated['max_sample'].apply(lambda x: 1 if (x >=20) else 0)
# count the number of unique patient that have sample_range = 1 per device
tdf = joined_updated[joined_updated['sample_range'] == 1].groupby(by=['device']).nunique()['session'].reset_index()
tdf.rename(columns={'session':'sample_range'}, inplace=True)
db_old = db_old.merge(tdf, left_on='device', right_on='device', how='outer')
...
# %%
db_old = db_old[['Manufacturer', 'Model', 'priority', 'device', 'Unique Sessions', 'Fitzpatrick_5_6', 'avg_sample', 'sample_range', 'min_sao2', 'max_sao2', 'so2_70-80', 'so2_80-90', 'so2_90-100']]
# fill zeroes
db_old.fillna(0, inplace=True)
...
column_dict_db_old = {'device':'Device',
                ...
                'Fitzpatrick_5_6':'Fitzpatrick V or VI',
