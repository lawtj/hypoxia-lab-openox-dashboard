...
def create_figures(konica, session, fig_size):
    ...
    konica_unique_median = konica_sub.groupby(['session', 'group']).median(numeric_only=True).reset_index()
    # calculate the ITA
    konica_unique_median['ita'] = konica_unique_median.apply(hypoxialab_functions.ita, axis=1)
    konica_unique_median = konica_unique_median.drop(['lab_l', 'lab_a', 'lab_b'], axis=1)
    konica_unique_median = konica_unique_median.dropna(subset=['ita'])
    ...
    joined_konica_session = pd.merge(session_sub, konica_unique_median, on='session', how='left')
